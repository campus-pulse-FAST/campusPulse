...
function CreateEventContent() {
  ...
  useEffect(() => {
    if (token) {
      api<{ data: Category[] }>('/categories', { token }).then((r) => setCategories(r.data));
      api<{ data: Venue[] }>('/venues', { token }).then((r) => setVenues(r.data));
    }
  }, [token]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await api<{ data: Event }>('/events', {
        method: 'POST',
        token: token!,
